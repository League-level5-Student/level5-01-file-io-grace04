...
public class ColorSelectionPanel extends JPanel implements MouseListener, ChangeListener, Serializable{
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(buttonClicked.equals(saveButton)) {
			try (FileOutputStream fos = new FileOutputStream(new File("pixelart.txt")); ObjectOutputStream oos = new ObjectOutputStream(fos)) {
				oos.writeObject(colorLabel);
			} catch (IOException a) {
				a.printStackTrace();
			}
		}
		if(buttonClicked.equals(loadButton)) {
			File file = new File("pixelart.txt");
			if(file.length()!=0) {
			try (FileInputStream fis = new FileInputStream(new File("pixelart.txt")); ObjectInputStream ois = new ObjectInputStream(fis)) {
				colorLabel.setIcon((Icon) ois.readObject());
			} catch (IOException a) {
				a.printStackTrace();
